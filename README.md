...
2. [Documentation](#2-documentation)
   ...
   - [2.15 LHP Bond Overlay — Unrepressed Sovereigns](#215-lhp-bond-overlay--unrepressed-sovereigns-allocationsbond_inputspy)
...
├── allocations/
│   ├── __init__.py      # lazy-export façade (equity + bond names)
│   ├── country_inputs.py # COUNTRY_INPUTS: per-market drift/CAPE/FX, sourced from §3.1 Table 1a
│   ├── mosaic.py        # build_equity_specs(), four canonical equity allocations
│   ├── bond_inputs.py   # BOND_INPUTS, build_overlay_specs() — LHP unrepressed-sovereign overlay
...
### `GovernmentBondSleeve` (`assets/em_bonds.py`)

Generic sovereign bond sleeve that **generalises** `ChinaGovernmentBondSleeve` to any sovereign — developed or emerging.  Accepts all the same O-U + global-pass-through parameters plus two additional arguments:

| Argument | Default | Description |
| --- | --- | --- |
| `fx_key` | `None` | FXModel currency key for the currency of the bond. `None` = EUR-denominated |
| `fx_exposure` | `1.0` | Unhedged fraction (set < 1 if partially hedged) |

The key design dial is `global_rate_beta`: the sensitivity of this sovereign's yield to changes in the EUR long rate.

- **High beta** (0.7–1.0) — tracks EUR rates closely; a EUR financial-repression episode is imported into this curve, eroding the real yield.
- **Low beta** (0.05–0.20) — a **decoupled, unrepressed curve** that follows its own fair real yield.  This is the §8.7 decoupling thesis: commodity-exporting sovereigns (Australia, New Zealand) and Asian EM sovereigns (Indonesia, India) have not imported European rate suppression historically.

```python
from assets.em_bonds import GovernmentBondSleeve

# Indonesia 10y — ~4% real yield, FX unhedged, decoupled from EUR repression
GovernmentBondSleeve(
    name="Indonesia",
    duration=7.0,
    initial_yield=0.066,
    long_run_yield=0.066,
...
Pre-calibrated instances for six sovereigns are available via `allocations.build_overlay_specs()` (see §2.15).
...
r_RSP = SubPortfolio(rsp_specs, fx_model=fx_model).step(state_t, state_t1)   — RSP sleeves + FX overlay
r_LHP = SubPortfolio(lhp_specs, fx_model=fx_model).step(state_t, state_t1)   — LHP sleeves + FX overlay
```
...
Both RSP and LHP `SubPortfolio` instances share the same `FXModel` (one RNG draw per currency per period). EUR-only sleeves carry no `fx_exposures` entries, so passing the FX model to the LHP is a no-op for the default EUR core. When the LHP holds an unrepressed-sovereign overlay (§2.15), the FX model delivers the currency P&L for those unhedged positions automatically.
...
## 2.15 LHP Bond Overlay — Unrepressed Sovereigns (`allocations/bond_inputs.py`)

### Why an unrepressed overlay?

The EUR-denominated LHP core (nominal govvies + linkers + cash) is fully exposed to a scenario where the ECB suppresses real yields below zero for an extended period — the financial-repression thesis associated with Russell Napier.  The bond-side overlay adds **sovereign bonds of countries that have historically not imported European rate suppression**, holding them unhedged in local currency so that a weaker EUR delivers an additional FX tailwind (via the FXModel).

Hedging back to EUR would, by covered-interest parity, drag the return back toward the EUR base rate and defeat the purpose.

### Three blocks

| Block | Countries | Rationale |
| --- | --- | --- |
| `DEVELOPED` | Australia, New Zealand | AAA/AA commodity exporters; positive real yields; independent central banks |
| `EM` | Indonesia, India, Korea | High real yields (~1.5–4 %); healthy balance sheets; curves historically decoupled from EUR |
| `MANAGED` | China (CGB) | Not claimed as "unrepressed"; included as a **capped dial** (default 0%) on store-of-value grounds |

### `bond_inputs.py` — calibration inputs

```python
from allocations import BOND_INPUTS, DEVELOPED, EM, MANAGED

# BOND_INPUTS: dict[str, dict] with keys:
#   block, within (intra-block share), yld (long_run_yield),
#   dur (duration), beta (global_rate_beta), idio (idio_yield_vol), fx (FXModel key)
```

| Sovereign | Block | Yield | Duration | β (EUR pass-through) | FX key |
| --- | --- | ---: | ---: | ---: | --- |
...
# Default: dev 10%, EM 10%, China 0%  (total overlay 20%; EUR-core residual 80%)
overlay = build_overlay_specs(dev_weight=0.10, em_weight=0.10, china_weight=0.0)

# With a 2% China dial:
overlay = build_overlay_specs(china_weight=0.02)   # total overlay 22%
```

The caller supplies the EUR-core residual (`1 − total_overlay_weight`).  `examples/run_attribution.py` shows the standard wiring pattern:
...
def build_lhp_with_overlay(dev=0.10, em=0.10, china=0.0):
    ...
    return eur_core + overlay
```

### Nested attribution (`examples/run_attribution.py`)

Four waypoints, each adding one decision on top of the previous:

| Step | Equity | LHP |
| --- | --- | --- |
| (i) Current | ACWI-like | EUR-only |
| (ii) +Re-anchor | Asia ~43%, China uncapped | EUR-only |
| (iii) +China cap | Proposed A (China 6%) | EUR-only |
| (iv) +Bond side | Proposed A | 80% EUR core + 10% dev + 10% EM overlay |
...
python -m examples.run_attribution 800        # more scenarios
```

Output: a marginal-contribution table showing the baseline and repression real-pot p50 at each step.  The bond-side step is designed to add little at baseline (its cost is low) and materially more under repression (its conditional value).

### Repression calibration
