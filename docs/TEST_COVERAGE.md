...
| V-M7 | Major | `test_repression_ramp_removes_the_boundary_step`, `test_bond_sleeve_single_year_returns_are_bounded` | ✅ **RESOLVED and fully guarded** (2026-08-22). The ramp guard asserts the pinned interior exactly, bounds mean transition-year LHP returns at ±35%, and caps the transition tail at 1.75× baseline; verified to fail on `ramp=0` |
